...
public class Transcript {
    ...
    //used for looking up a string in the transcript, and finding its corresponding timeregions
    private final Map<StartEndIndex, TimeRegion> indexToTimeRegion = new LinkedHashMap<>();
            ...
            while ((currentLine = bufferedReader.readLine()) != null) {
                ...
                if (counter % 2 == 0) {
                    ...
                    */
                    ...
                    TimeRegion toAdd = new TimeRegion(new TimeRange(startTime, endTime), currentLine);
                    timeRegionList.add(toAdd);

                    startTime = null;
                    endTime = null;
                    transcriptWordsWithoutTimes.append(currentLine).append(" ");

                    int newStringIndex = stringIndex + currentLine.length() + 1;
                    indexToTimeRegion.put(new StartEndIndex(stringIndex, newStringIndex), toAdd);
...
//            }
    ...
    private static List<TimeRegion> createTimeRegionsBasedOnSentences(List<String> allSentences, List<TimeRegion> timeRegionList) {
        ...
        for (String sentence : allSentences) {
            ...
            int startSeconds = TimeUtils.calculateSeconds(startTimeRegion.getStartTime())
                    + (int)
                    (
            ...
            if (portionOfSentenceLeft.length() == 0) {
                ...
                int endSeconds = TimeUtils.calculateSeconds(endTimeRegion.getStartTime())
                        + (int)
                        (
    ...
    /**
     * returns 1 or more adjacent Timeregions whose captionstrings
     * encapsulate the sentence that was entered. If the sentence is not unique,
     * i.e. occurs multiple times in the transcript,
     * the first matching set of Timeregions will be returned.
     */
    public List<TimeRegion> stringToTimeRegions(String input){
        int startIndex = transcriptWithoutTimeValues.indexOf(input);
        int endIndex = startIndex + input.length();
        StartEndIndex inputIndices = new StartEndIndex(startIndex, endIndex);
        List<TimeRegion> toReturn = new ArrayList<>();
        /**
         * assumes the indexToTimeRegion map is sorted (i.e. a linkedhashmap)
         */
        boolean startedIntersecting = false;
        for (StartEndIndex startEndIndex : indexToTimeRegion.keySet()) {
            if (startEndIndex.intersects(inputIndices)) {
                toReturn.add(indexToTimeRegion.get(startEndIndex));
